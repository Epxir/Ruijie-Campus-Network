...
def encryptedPassword(password):
 js_code = """
    ...
    function encryptedPassword(password,macStringTemp,publicKeyExponent,publicKeyModulus){
        console.log(password,macStringTemp,publicKeyExponent,publicKeyModulus)
        var passwordMacTemp = password+'>'+macStringTemp;
        var passwordEncode = passwordMacTemp.split('').reverse().join('');
        var publicKeyExponent= publicKeyExponent;
        var publicKeyModulus= publicKeyModulus;
        RSAUtils.setMaxDigits(400);
        var key = new RSAUtils.getKeyPair(publicKeyExponent, '', publicKeyModulus);
        var passwordEncrypt = RSAUtils.encryptedString(key,passwordEncode);
 ...
 publicKeyExponent = loads(response.text)['publicKeyExponent']
 publicKeyModulus = loads(response.text)['publicKeyModulus']
 encryptedPassword = ctx.call('encryptedPassword',password,macStringTemp,publicKeyExponent,publicKeyModulus)
